    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            ...
            authorities.forEach(authority -> {
                log.info("==============peihncn==={}", authority);
            });
//            if (!authorities.isEmpty()) {
//                authorities.add("PROFILE"); //by default, it should be there
//                authorities.add("USER_ADMIN"); //by default, it should be there
//            }
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                .securityMatcher("/swagger-ui/**", "/v3/api-docs**", "/api-docs/**", "/api-docs-ui.html",
                        "/.well-known/**", "/state/health", "/state/info", "/profile/**","/roles/**")
